      ...
      columnDefs: [{
        ...
        },
        {
          ...
          "render": function(data, type, row, meta) {
            ...
            let status_ujian;
                if (
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.user_asesi_id == {!! json_encode(Auth::user()->id) !!} && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.jadwal_uji_kompetensi_id == row.jadwal_uji_kompetensi_id && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.status_ujian_berlangsung == 0) {
                  status_ujian = `<span class="text-black my-1"> Belum Memulai Ujian</span>`
                }
                else if (
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.user_asesi_id == {!! json_encode(Auth::user()->id) !!} && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.jadwal_uji_kompetensi_id == row.jadwal_uji_kompetensi_id && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.status_ujian_berlangsung == 1){
                  status_ujian = `<span class="text-black my-1">Sedang Berlangsung</span>`
                }
                else if (
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.user_asesi_id == {!! json_encode(Auth::user()->id) !!} && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.jadwal_uji_kompetensi_id == row.jadwal_uji_kompetensi_id && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.status_ujian_berlangsung == 2){
                  status_ujian = `<span class="text-black my-1">Selesai Pengerjaan</span>`
                }else if (
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.user_asesi_id == {!! json_encode(Auth::user()->id) !!} && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.jadwal_uji_kompetensi_id == row.jadwal_uji_kompetensi_id && 
                    row.relasi_jadwal_uji_kompetensi.relasi_user_login_asesi.status_ujian_berlangsung == 3){
        ...
        },
        {
          ...
          "render": function(data, type, row, meta) {
            ...
            if(row.waktu_mulai < moment().format('YYYY-MM-DD HH:mm:ss') && row.waktu_selesai > moment().format('YYYY-MM-DD HH:mm:ss')){
              ...
              row.waktu_mulai > moment().format('YYYY-MM-DD HH:mm:ss') && row.waktu_selesai > moment().format('YYYY-MM-DD HH:mm:ss')){
